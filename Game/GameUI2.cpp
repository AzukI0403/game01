...
bool GameUI2::Start()
{
	...
	//表示する座標を設定する。
	fontRender_time.SetPosition({ -84.0f,540.0f,0.0f });
	//y=-400
...
void GameUI2::Update()
{
	...
	//boss02のインスタンスを見つけたかつboss02が倒されていないなら。
	if (bossdiscover == 1 && bossdeath == 0)
	{
		//HPバーの減っていく割合。
		float X_BossSize = (float)boss02->bosshp / (float)boss02->maxbosshp;
...
//game2にtimeの値を返す。
void GameUI2::SetResultDataToGame() {
	game2->time_rinzi = time;

}

//warning_barのスプライトを動かす処理。
void GameUI2::Warning()
{
	if (musicON == false)
	{
		//bossが出現するときの音を再生する。
...
void GameUI2::Render(RenderContext& rc)
{
	if (ms_btime <= 100)
	{
		//上下のバー
